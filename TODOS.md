...
- [ ] **Make `swift test` work on the macOS host.** Multiple files use iOS-only APIs without `#if canImport(UIKit)` / `#if os(iOS)` guards, blocking `swift test` locally. Known offenders: `Sources/Managers/ImagePicker.swift` (uses `uiImage:` label that doesn't exist on macOS), `Sources/Components/Buttons/AddToWalletButton.swift` (`AddPassToWalletButton` is iOS-only), `Sources/Examples/ShoppingCart/**` (uses `StackNavigationViewStyle` which is iOS-only). Either guard them properly, or add `Examples` to the target's `exclude` array in `Package.swift` and guard the others. `ColorHelper.swift` was already fixed using `#if canImport(UIKit)`.
