...
- [ ] **Full multi-platform audit for watchOS / tvOS.** Many files use APIs unavailable on watchOS (`segmented`, `GroupBox`, `NavigationView` styles) or tvOS (PassKit, LocalAuthentication, various UIKit pickers) without `#if` guards. CI's `xcodebuild watchOS` and `xcodebuild tvOS` jobs are currently marked `continue-on-error: true` until the audit lands. Same applies to the `Periphery` job (will surface significant dead code in the existing helper classes — needs a triage pass first). Hard CI gates remain: `swift test`, `xcodebuild iOS`, `SwiftLint`.
